...
#### `paste-resources-into-prop-group`

Spreads existing CMS resources across the props of one or more prop GROUPS (ids from get-room-prop-groups), in the room's navigation order, as contiguous, order-preserving shares — one slideshow per prop. The tool does ALL the chunking (7 resources over 3 frames -> 3/2/2; 2 over 5 -> the first two frames get one each), so call it ONCE per content group with all of that group's resources — never once per prop, and never split the resources yourself. Pass several groupIds when the content is coarser than the room's groups (e.g. "team" photos over "building1+floor0+team" and "building1+floor1+team"); their props are pooled in walk order. distribution: "spread" (default — contiguous shares), "stack" (everything onto the first eligible prop as one slideshow — only when the user explicitly asked for a single slideshow) or "replicate" (EVERY resource onto EVERY eligible prop — the default when every selected group is a LOGO group, isLogoGroup true, so all logos show on all logo frames; never spread logos unless the user asks). Presentation boards are skipped automatically and listed in skippedProps; decorative members (walls, signs) are never candidates. Per prop this behaves exactly like paste-resources-into-prop: resources are link-copied (originals stay), template placeholder stubs are removed first, and the auto content slider is enabled when a prop ends up with more than one item. Returns { roomId, groupIds, distribution, placements: [{ groupId, propId, name, tag, navIndex, resourceIds, addedResourceIds, failed, removedTemplateStubs, sliderEnabled, sliderError?, stubCleanupError?, error? }], unplacedResourceIds (resources that landed on no prop at all), eligiblePropCount, skippedProps }. It throws only when nothing could be attempted: an unknown group id (the message lists the valid ids), no eligible prop in the selected groups, or empty input. Report failed/unplaced items honestly; paste each placement's `tag` verbatim when linking. Use paste-resources-into-prop only when the user named exactly one prop.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `roomId` | string | Yes | The room the groups belong to. |
| `groupIds` | string[] | Yes | One or more prop group ids from get-room-prop-groups (e.g. ["building1+floor0+team"]). Several ids pool their props into one navigation-ordered row. |
| `resourceIds` | string[] | Yes | CMS resource ids to place, in display order. The tool splits them across the props itself. |
| `distribution` | string | No | "spread": contiguous, order-preserving shares across the eligible props in navigation order (default for every group except logo groups). "stack": everything onto the first eligible prop as a single slideshow — only when the user asked for one. "replicate": every resource onto every eligible prop (default for a logo group). Omit to get the default for the selected groups. |
| `appendToListEnd` | boolean | No | When true (default) new resources are appended to the end of each prop's content list; when false they are inserted at the front. |
| `removeTemplateStubs` | boolean | No | When true (default) placeholder resources a prop inherited from the room template are removed before pasting. Pass false only when the user explicitly wants to keep the template content. |
| `networkId` | string | No | Network ID the room belongs to. Defaults to the current network. |

#### `rename-prop-group`

Gives a prop GROUP a display name: sets the title of every CONTENT prop in the group (frames, boards) and of the group's section signs (its labels from get-room-prop-groups — the tagged Text props showing the section name in the room) through a full-object save, so the 3D sign and the frames read e.g. "Leadership Team"; untitled decorative members (walls, plants) are left alone. Use it in the guided build once the section's name is settled (listed by the user up front, self-evident from the tag, or confirmed from your proposal — or chosen by you when the user asked you to decide everything). The group id itself comes from tags and does not change. Returns { roomId, groupId, name, renamedPropIds, renamedLabelPropIds, failed } — renamedLabelPropIds are the signs that now show the name; failed lists props that could not be saved. Never rename a group the user did not name or ask you to name.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `roomId` | string | Yes | The room the group belongs to. |
| `groupId` | string | Yes | The prop group id from get-room-prop-groups (e.g. "team" or "building1+floor0+team"). |
| `name` | string | Yes | The display name for the section; becomes the title of the group's content props and the text of its signs. |
| `renameLabels` | boolean | No | When true (default) the section's signs (labels) are retitled too. Pass false only when the user wants the sign text left as it is. |
| `networkId` | string | No | Network ID the room belongs to. Defaults to the current network. |
