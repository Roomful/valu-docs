...
Gives a prop GROUP a display name: sets the title of EVERY prop in the group (frames and any tagged label/sign props alike) through a full-object save, so the section reads as e.g. "Leadership Team" in the room. Use it in the guided build right after the user confirms the section name you proposed from their content (or, when the user asked you to decide everything, with the name you chose). The group id itself comes from tags and does not change. Returns { roomId, groupId, name, renamedPropIds, failed } — failed lists props that could not be saved, the rest were renamed. Never rename a group the user did not name or ask you to name.
...
| `name` | string | Yes | The display name for the section; becomes the title of every prop in the group. |
| `networkId` | string | No | Network ID the room belongs to. Defaults to the current network. |
